...
module Jekyll
  ...
  class Tagger < Generator
    ...
    def calculate_tag_cloud(num = 5)
      ...
      tags = site.tags.map { |tag, posts, nump|
        [tag.to_s, range < (size = posts.size) ? range = size : size, posts.size]
      }

      range = 1..range

      tags.sort!.map! { |tag, size, nump| [tag, range.quantile(size, num), size.to_s] }
    end
  ...
  module Filters
    ...
    def tag_cloud(site)
      site['tag_data'].map { |tag, set, nump|
        tag_link(tag, tag_url(tag), {:class => "set-#{set}", :title => "###CONTAINS### #{nump} ###POSTS###"})
      }.join(' ')
    end
